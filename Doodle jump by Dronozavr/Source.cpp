...
void RecordsMenu(SDL_Renderer*& renderer, int& IsSound, int& IsMusic) // Добавить кнопку back
{
	TTF_Init();
	...
	while (!quit)
	{
		...
		while (SDL_PollEvent(&event))
		{
				...
				event.button.button == SDL_BUTTON_LEFT)
			{
				...
				if (SDL_HasIntersection(&nowmouse, &ForButtonRecords)) // СОЗДАТЬ НОВУЮ ФУНКЦИЮ
				{
